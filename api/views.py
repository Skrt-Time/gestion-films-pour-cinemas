...
class PublicFilmListAPI(APIView):
    """
    GET /api/public/films/?search=Avatar
    Affiche tous les films proposés dans une ville donnée OU par titre.
    """
    def get(self, request):
        # On accepte 'search' ou 'ville' (pour rétrocompatibilité)
        query = request.query_params.get('search') or request.query_params.get('ville')
        films = services.list_movies_service(search_filter=query)
...
class CinemaFilmCreateAPI(APIView):
    ...
    def post(self, request, cinema_id):
        ...
        if existing_film:
            # On met à jour les infos si nécessaire
            serializer = FilmSerializer(existing_film, data=data, partial=True)
            if serializer.is_valid():
                serializer.save()
